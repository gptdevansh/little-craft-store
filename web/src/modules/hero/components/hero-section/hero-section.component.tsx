import { Button, Icon } from '@/shared';
import Image from 'next/image';

export function HeroSection() {
  return (
    <section className="px-[var(--spacing-container-margin)] py-8 md:py-20 relative flex flex-col md:flex-row items-center gap-6 md:gap-[var(--spacing-gutter)] max-w-7xl mx-auto">
      {/* Botanical accent — desktop only */}
      <div className="absolute top-10 left-6 botanical-accent z-0 pointer-events-none hidden md:block opacity-40">
        <Icon name="psychiatry" size={40} className="font-thin" />
      </div>

      {/* Text Content */}
      <div className="w-full md:w-1/2 flex flex-col items-start gap-4 md:gap-6 z-10 pt-2 md:pt-4">
        <h1 className="font-[family-name:var(--font-headline)] text-[24px] sm:text-[32px] md:text-[40px] leading-[1.2] tracking-tight text-primary font-semibold">
          Handmade hair accessories, made with love.
        </h1>
        <p className="font-[family-name:var(--font-body)] text-[15px] md:text-[18px] leading-[24px] md:leading-[28px] text-on-surface-variant max-w-md font-light">
          Discover our collection of artisanal scrunchies, crafted with premium
          fabrics and thoughtful details to elevate your everyday style.
        </p>
        <Button href="/shop" variant="cta" size="lg" className="w-full sm:w-auto text-center">
          Browse Collection
        </Button>
      </div>

      {/* Hero Image */}
      <div className="w-full md:w-1/2 relative mt-2 md:mt-0 z-10">
        <div className="relative aspect-[4/3] md:aspect-[4/4] rounded-[2rem] overflow-hidden border border-outline-variant/20 shadow-sm">
          <Image
            src="/img/hero/hero.jpg"
            alt="Handmade silk and velvet scrunchies in warm plum and blush tones on linen"
            fill
            sizes="(max-width: 768px) 100vw, 50vw"
            priority
            className="object-cover object-center"
          />
        </div>
      </div>
    </section>
  );
}
